...
export const UserMenu = ({ user, onSignOut }: UserMenuProps) => {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        className="w-64 md:w-56 bg-popover border border-border shadow-lg z-[9999]"
        align="end"
        forceMount
        side="bottom"
        sideOffset={12}
      >
        <div className="flex items-center justify-start gap-2 p-3 md:p-2">
          <div className="flex flex-col space-y-1 leading-none">
            {user.user_metadata?.full_name && (
              <p className="font-medium text-base md:text-sm text-foreground">{user.user_metadata.full_name}</p>
            )}
        ...
        <DropdownMenuItem
          onClick={onSignOut}
          className="cursor-pointer p-3 md:p-2 min-h-[44px] touch-manipulation text-destructive focus:text-destructive"
        >
